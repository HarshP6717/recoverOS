...
| **`payment_method_update`** | **Recommendation-Only Capability** | Simulated mandate session / portal guidance | ₹2.00 | Generates customer guidance and simulated update session token for replacing expired card/mandate details without cancelling subscription. (Public Razorpay test sandbox lacks direct mandate replacement endpoints). | **100% Demonstrable** (Generates simulated session URL). |
...
| **`send_reminder`** | **Tier C: Comms Simulation** | SMS / WhatsApp notification simulation | ₹0.50 | Non-intrusive notification reminding customer of pending subscription renewal via SMS/WhatsApp with one-click payment. | **100% Demonstrable** (Notification payload generated). |
